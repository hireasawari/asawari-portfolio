...
function EditorialLabel({ icon: Icon, title, index, isEven }: { icon: any; title: string; index: number; isEven: boolean }) {
  return (
    <div className={`relative flex flex-col items-center justify-center gap-4 py-6 ${isEven ? "md:items-end md:text-right" : "md:items-start md:text-left"}`}>
      ...
      {/* Rule */}
      <div className={`h-px w-16 z-10 mt-1 bg-gradient-to-r ${isEven ? "from-transparent to-primary/50" : "from-primary/50 to-transparent"}`} />
    </div>
  );
}
...
// ─── Phone Frame ───────────────────────────────────────────────────────────────
function PhoneFrame({ src, label, dim = false, onClick, size = "md", padded = false, notch = false }: { src: string; label?: string; dim?: boolean; onClick?: () => void; size?: "sm" | "md" | "lg", padded?: boolean, notch?: boolean }) {
  let sizeClasses = "w-[260px] h-[560px]"; // ~2.16 screen ratio
    ...
    >
      <div className={`relative mx-auto ${sizeClasses}`}>
        <div className={`absolute inset-0 ${radiusClasses} border-4 overflow-hidden shadow-2xl ${dim ? "opacity-70 grayscale-[60%]" : ""}`}
          ...
          {notch && <div className={`absolute top-0 left-1/2 -translate-x-1/2 bg-black z-10 ${notchClasses}`} />}
          <img src={src} alt={label || ""} className={`w-full h-full object-cover object-top ${padded ? 'pt-6 bg-white' : ''}`}
            onError={(e) => {
...
function ScreenRow({ screen, index, onPreview }: { screen: DesignScreen; index: number; onPreview: (s: any) => void }) {
    ...
    <div className="relative flex flex-col justify-start w-full">
      ...
      <div className="flex justify-center pb-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          {screen.before ? (
            <PhoneFrame src={screen.before} size="md" dim padded={screen.src?.includes('home')} />
          ) : (
            <div className="w-[240px] h-[560px] rounded-[3rem] border-4 border-white/5 bg-neutral-900/40 flex flex-col items-center justify-center gap-3">
  ...
  const afterCol = (
    <div className="flex flex-col justify-start">
      <div className="relative rounded-2xl bg-surface-container-low border border-white/5 overflow-hidden transition-all duration-500 hover:border-primary/20 group/card">
        <div className="absolute inset-0 bg-gradient-to-br from-transparent to-transparent group-hover/card:from-primary/4 transition-all duration-500 pointer-events-none" />
        <div className="absolute top-0 bottom-0 left-0 w-1 transition-all duration-500 opacity-50 group-hover/card:opacity-100 bg-gradient-to-b from-primary/60 via-primary/20 to-transparent" />
        ...
        <div className="flex justify-center pb-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            <PhoneFrame src={screen.src} size="md" onClick={() => onPreview(screen)} notch={screen.src.includes('search') || screen.label.toLowerCase().includes('explore')} />
          </motion.div>
        </div>
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
const DigilockerCaseStudyPage = () => {
  const project = projectsData.find(p => p.id === "digilocker-redesign");
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute inset-0 opacity-[0.025]"
          style={{ backgroundImage: "radial-gradient(circle, #f9851f 1px, transparent 1px)", backgroundSize: "44px 44px" }} />
        <motion.div className="absolute -top-32 right-0 w-[600px] h-[600px] rounded-full"
          style={{ background: "radial-gradient(circle, rgba(249, 133, 31, 0.07) 0%, transparent 70%)" }}
        ...
        <motion.div className="absolute bottom-1/3 -left-32 w-96 h-96 rounded-full"
          style={{ background: "radial-gradient(circle, rgba(249, 133, 31, 0.05) 0%, transparent 70%)" }}
            ...
            <div className="grid md:grid-cols-2 gap-12 md:gap-20 items-center">
              ...
              <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}
                ...
                <motion.div animate={{ y: [0, -14, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}>
                  <PhoneFrame
                    src="/images/digilocker/landing.png"
                    label="Landing Screen"
                    padded={false}
                    onClick={() => setLightbox({ src: "/images/digilocker/landing.png", label: "Landing Screen", description: "", improved: [] })}
                  />
                </motion.div>
              </motion.div>
            </div>
        ...
        <div className="container max-w-7xl space-y-24 pt-4">
          ...
          <motion.section initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            ...
            <div className="grid md:grid-cols-[1fr_2px_3fr] gap-8 md:gap-16 items-start">
              ...
              <div className="hidden md:block w-px self-stretch bg-gradient-to-b from-transparent via-primary/15 to-transparent" />
              <div className="relative rounded-2xl bg-surface-container-low border border-white/5 p-6 md:p-8 overflow-hidden hover:border-primary/20 transition-all duration-500 group/card">
                <div className="absolute inset-0 bg-gradient-to-br from-transparent to-transparent group-hover/card:from-primary/4 transition-all duration-500 pointer-events-none" />
                <div className="absolute top-0 bottom-0 left-0 w-1 bg-gradient-to-b from-primary/60 via-primary/20 to-transparent opacity-50 group-hover/card:opacity-100 transition-all duration-500" />
          ...
          <motion.section initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            ...
            <div className="grid md:grid-cols-[3fr_2px_1fr] gap-8 md:gap-16 items-start">
              <div className="relative rounded-2xl bg-surface-container-low border border-white/5 p-6 md:p-8 overflow-hidden hover:border-primary/20 transition-all duration-500 group/card md:order-1">
                <div className="absolute inset-0 bg-gradient-to-br from-transparent to-transparent group-hover/card:from-primary/4 transition-all duration-500 pointer-events-none" />
                <div className="absolute top-0 bottom-0 right-0 w-1 bg-gradient-to-b from-primary/60 via-primary/20 to-transparent opacity-50 group-hover/card:opacity-100 transition-all duration-500" />
              ...
              <div className="hidden md:block w-px self-stretch bg-gradient-to-b from-transparent via-primary/15 to-transparent md:order-2" />
          ...
          <section className="relative">
            ...
            <div className="absolute inset-0 -mx-6 md:-mx-12 rounded-3xl overflow-hidden pointer-events-none flex items-center justify-center" aria-hidden="true">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-[8deg] w-[200%] flex flex-col gap-12 md:gap-24 text-[12vw] md:text-[130px] font-display font-black whitespace-nowrap text-white/[0.015] select-none pointer-events-none leading-none">
                <div className="ml-[-5%]">DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER</div>
                ...
                <div className="ml-[-2%]">DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER</div>
                ...
                <div className="ml-[-8%]">DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER</div>
                ...
                <div className="ml-[-4%]">DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER •   • DIGILOCKER</div>
              </div>
              ...
              <div className="absolute bottom-[20%] right-[-5%] w-[600px] h-[600px] rounded-full blur-[150px] opacity-10" style={{ background: BK }} />
                <div className="absolute top-[60%] left-[20%] w-[400px] h-[400px] rounded-full blur-[100px] opacity-[0.15]" style={{ background: "#f9851f" }} />
              {/* Grid pattern */}
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {d.solution.map((item, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true }} transition={{ duration: 0.45, delay: i * 0.07 }}
                      className="flex items-start gap-3 rounded-xl border border-white/5 bg-surface-container-high/40 p-4 hover:border-primary/20 transition-colors">
              ...
              <div className="flex flex-wrap gap-3 py-4">
                {d.designHighlights.map((h, i) => (
                  <motion.span key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }}
                    ...
                    className="px-5 py-2.5 rounded-full bg-surface-container-low border border-white/8 text-sm font-medium text-foreground/75 hover:border-primary/30 hover:text-foreground transition-all cursor-default">
                    {h}
                  </motion.span>
                ))}
              </div>
...
export default DigilockerCaseStudyPage;
